...
export const patchROM = (asset, isoFile, destFile, version, clear, store, enqueueSnackbar, closeSnackbar, index) => {
    if (!asset) return "Error";

    if (!isoFile)
        isoFile = store.get("vanillaIsoPath");

    var lastSnackbar = undefined;
    const showInfo = (message) => enqueueSnackbar && enqueueSnackbar(message, { variant: 'info', autoHideDuration: null, anchorOrigin: {horizontal: 'right', vertical: 'top'}, action: (key) => { lastSnackbar = key; }});

    return new Promise(async (resolve, reject) => {
    try {
        showInfo("Downloading patch file...");
        const res = await fetch(asset.downloadUrl, {redirect: "follow", headers: {
            ...
            fs.readFile(isoFile, (err, isoFileBuffer) => {
                ...
                worker.addEventListener('message', (e) => {
                    ...
                    try {
                        ...
                        enqueueSnackbar && enqueueSnackbar("All steps completed!", { variant: 'success', anchorOrigin: {horizontal: 'right', vertical: 'top'}})
                        resolve();
                        });
                    } catch(error) {
                        reject(error);
